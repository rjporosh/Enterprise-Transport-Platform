...
| M7 Notification production-safe | ✅ Done (2026-09-03) — EF Core 10.0.0 + Quartz 3.14; `CoreTemplateSeeder` seeds 18 templates (9 events × en + bn) incl. `ticket.issued` with `{{pdfUrl}}` link; Bangladesh SMS provider (`BdSmsSender`) behind `ISmsSender`; `NotificationEventConsumer` maps `ticket.issued`→email + extracts `EventId` for unique `SourceReference`; `SendNotificationHandler` inbox-dedup (`AnyAsync` on `SourceReference`); unique filtered DB index; `NotificationDispatchJob` claim-then-send (MarkSending+save before channel, result-save after); `.RequireAuthorization()` on all notification endpoints; 2 EF migrations; **29/29 unit tests green; 0 errors 0 warnings**. | feat(notification): M7 — production-safe delivery, EF Core 10, BD SMS, ticket.issued consumer, inbox dedup, authz |
