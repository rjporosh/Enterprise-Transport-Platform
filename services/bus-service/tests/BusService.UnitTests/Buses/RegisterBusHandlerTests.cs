...
public class RegisterBusHandlerTests : IDisposable
{
    ...
    private RegisterBusHandler CreateHandler() => new(_context, _eventPublisher, _clock, _metrics);

    [Fact]
    public async Task Handle_WithNewPlateNumber_RegistersBus()
    {
        var handler = CreateHandler();
        var command = new RegisterBusCommand(Guid.NewGuid(), "DL-1PC-1234", "AcSleeper", 40, _depotId, "Volvo", "9600", 2022);
    ...
    {
        var handler = CreateHandler();
        var command = new RegisterBusCommand(Guid.NewGuid(), "DL-1PC-1234", "AcSleeper", 40, Guid.NewGuid(), null, null, null);
    ...
    {
        var handler = CreateHandler();
        await handler.Handle(new RegisterBusCommand(Guid.NewGuid(), "DL-1PC-1234", "AcSleeper", 40, _depotId, null, null, null), CancellationToken.None);

        var act = async () => await handler.Handle(new RegisterBusCommand(Guid.NewGuid(), "dl-1pc-1234", "NonAcSeater", 45, _depotId, null, null, null), CancellationToken.None);
